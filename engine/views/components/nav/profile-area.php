...
<div class="profile-area">
    <?php if ($_SESSION['user']): ?>
        <?php if ($_SESSION['user']['profile_image']): ?>
            <img class="user-circle-img dropdown-toggle" src="<?= $_SESSION['user']['profile_image'] ?>"
                 ...
                 title="<?= $username ?>">
        <?php else: ?>
            <img src="/assets/uploads/icons/profile/profile-<?= $_SESSION['user']['gender'] ?>-default.jpg"
                 alt="<?= $username ?>"
                 id="navbarDropdown-profile" role="button"
        ...
        <div class="dropdown-menu" style="right: 0;" aria-labelledby="navbarDropdown-profile">
            ...
            <a class="dropdown-item" href="/profile/user/<?= $id ?>/profile-page.html?tab=add-public">Добавить
                публикацию</a>
    ...
    <?php else: ?>
        <img class="nav-link dropdown-toggle" style="display: inline-block"
             src="/assets/uploads/icons/profile/log-in.jpg" alt="Иконка входа"
             id="navbarDropdown-profile" role="button"
             data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
